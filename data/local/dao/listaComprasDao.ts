...
export async function inserirAtualizarItemListaCompras(
    ...
    for (const item of items) {
        ...
        const itemExistente = await getFirstAsync<any>(
            `SELECT usuario_id, ingrediente_id, marcado, precisa_sincronizar, local, atualizado_em, deletado_em
             FROM ${TABLE_NAME}
        ...
        if (itemExistente) {
            // Atualiza apenas marcado, precisa_sincronizar, local e timestamp
            await db.runAsync(
                `UPDATE ${TABLE_NAME}
             SET marcado = ?, precisa_sincronizar = ?, local = ?, atualizado_em = ?
            ...
            await db.runAsync(
             ...
             (usuario_id, ingrediente_id, marcado, precisa_sincronizar, local, atualizado_em)
             VALUES (?, ?, ?, ?, ?, ?)`,
                [
                    usuario.id,
                    item.ingrediente_id,
